...
input_BSC = Input(shape=(Input_train.shape[1], Input_train.shape[2], 1))

# input_BSC = RandomFlip(mode='horizontal_and_vertical')(input_BSC)
input_BSC = RandomContrast(factor=0.5)(input_BSC)

Layer_conv = Convolution2D(4, (2, 2), padding='same',activation='relu')(input_BSC)
...
output_BSC = Dense(5, activation='sigmoid')(Layer_flat)

model_BSC = Model(inputs=input_BSC, outputs=output_BSC)

# %%
# Compile the model

model_BSC.compile(optimizer='adam', loss='mean_squared_error', metrics=['accuracy'])

# %%
# Train the model

history = model_BSC.fit(Input_train,
                        Output_train_normed[:, idx_BSC],
                        epochs=100,
                        batch_size=64)
